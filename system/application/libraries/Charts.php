...
class Charts {
    ...
    // Added by Martino Visintin (EWB-UK 2010 : email martino.visintin@ewb-uk.org)
    // Draws a nice looking bar chart, not the crappy implementation of the cartesian shit
    // at least that was the idea.. no time to do it
    function barChart($x,$y, $title)
    {
        // Dataset definition   
        $DataSet = new pData;

        // Add the bars with their values
        $i = 1;
        foreach ($x as $bar)
        {
            $DataSet->AddPoint($bar, "Serie".$i);  
            $i++;
        }
        ...
        // Name the bars for the legend
        $i = 1;
        foreach ($y as $label)
        {
            $DataSet->SetSerieName($label, "Serie".$i);
            $i++;
        } 

        // Initialise the graph  
        $Test = new pChart(800,230);  
        $Test->setFontProperties($this->_ext_path."/fonts/"."tahoma.ttf",8);  
        ...
        // Draw the 0 line  
        $Test->setFontProperties($this->_ext_path."/fonts/"."tahoma.ttf",6);  
        ...
        $Test->setFontProperties($this->_ext_path."/fonts/"."tahoma.ttf",10);  
        $Test->drawTitle(50,22,$title,50,50,50,585); 

        // Create a random image name
        $this->obj->load->helper('string');
        $imgname = $this->_img_path.'/'.time().".png";
        $Test->Render($imgname);
        return  array("name" => '/'.$imgname,"w" => 800, "h" => 230);
    }
